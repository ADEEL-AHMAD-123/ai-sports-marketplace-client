...
import { setActiveFilter } from '@/store/slices/uiSlice';
...
const FILTERS = [
  { key: 'all',            label: 'All Props',       hint: '',                  Icon: AllIcon   },
  { key: 'highConfidence', label: 'High Confidence', hint: '8/10+ hit rate',    Icon: TrendIcon },
  { key: 'bestValue',      label: 'Best Value',      hint: '15%+ edge on line', Icon: BoltIcon  },
];
...
export default function MatchPage() {
  ...
  const { props, isLoading, error, refresh, activeFilter } = useProps(sport, eventId);
    ...
    <div className={styles.page}>
      <div className={styles.container}>
        ...
        <div className={styles.header}>
          ...
          <button className={styles.refreshBtn} onClick={refresh}><RefreshIcon />Refresh</button>
        </div>
        ...
        {isLoading && <div className={styles.grid}>{[...Array(6)].map((_, i) => <div key={i} className={styles.skCard}><div className={styles.skPulse} /></div>)}</div>}

        {error && <div className={styles.empty}><p className={styles.emptyTitle}>Could not load props</p><p className={styles.emptySub}>Backend must be running. <button onClick={refresh}>Retry →</button></p></div>}
        ...
        {!isLoading && !error && props.length === 0 && (
          <div className={styles.empty}>
            ...
            <p className={styles.emptySub}>{activeFilter !== 'all' ? 'Try "All Props".' : 'Props are fetched every 30 minutes.'}</p>
          </div>
        )}

        {!isLoading && !error && props.length > 0 && (
          <AnimatePresence mode="wait">
            <motion.div key={`${eventId}-${activeFilter}`} className={styles.grid} variants={stagger} initial="hidden" animate="show">
              {props.map(p => (
                <motion.div key={`${p.playerName}-${p.statType}`} variants={item}>
