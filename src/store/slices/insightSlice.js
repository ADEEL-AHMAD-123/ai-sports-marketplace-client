...
const initialState = {
  ...
  // Per-prop injury blocks returned by unlock endpoint (422)
  blockedInsights: {},
...
const insightSlice = createSlice({
  ...
  reducers: {
    ...
    clearBlockedInsight(state, { payload: key }) {
      if (key) delete state.blockedInsights[key];
    },
    ...
    builder
      .addCase(unlockInsight.pending, (state, { meta }) => {
        ...
        if (key) {
          ...
          delete state.blockedInsights[key];
        }
      })
      .addCase(unlockInsight.rejected, (state, { payload, meta }) => {
        ...
        if (key) {
          ...
          const injuryInfo = payload?.details?.injuryInfo || payload?.injuryInfo;
          if (payload?.status === 422 && injuryInfo?.skip) {
            state.blockedInsights[key] = {
              ...injuryInfo,
              message: payload?.message || 'Insight blocked due to player injury status.',
            };
          }
        }
      })
...
export const { clearUnlockError, clearBlockedInsight, cacheInsight } = insightSlice.actions;
...
export const selectBlockedInsightKey = (key) => (s) => s.insights.blockedInsights[key] || null;
